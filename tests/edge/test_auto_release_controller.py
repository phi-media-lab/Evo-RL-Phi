...
from lerobot.cloud.materializer import FilesystemEpisodeMaterializer, MaterializerHTTPServer
...
def test_auto_release_daemon_can_trigger_http_materializer(tmp_path):
    robot = make_robot_from_config(
        MockRobotConfig(
            n_motors=3,
            random_values=False,
    ...
    try:
        loop = AutoReleaseDaemon(
            ...
            poll_interval_s=0.0,
            sleep_fn=lambda _: None,
            materializer_base_url=f"http://127.0.0.1:{server.server_port}",
            materializer_repo_id="local/auto-release-http",
            materializer_fps=20,
        )
    finally:
        server.shutdown()
    ...
    assert loop.results[0].action == "released"
